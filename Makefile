...
NETWORK_ARGS := http://localhost:8545 --private-key $(DEFAULT_ANVIL_KEY) --broadcast

ifeq ($(findstring --network sepolia,$(ARGS)),--network sepolia)
	NETWORK_ARGS := --rpc-url $(SEPOLIA_RPC_URL) --private-key $(PRIVATE_KEY) --broadcast --verify --etherscan-api-key $(ETHERSCAN_API_KEY) -vvvv
endif

ifeq ($(findstring --network dfktest,$(ARGS)),--network dfktest)
	NETWORK_ARGS := --rpc-url $(DFKTEST_RPC_URL) --private-key $(PRIVATE_KEY) --broadcast --verify
endif
...
dfk:
	@forge create --rpc-url $(DFKTEST_RPC_URL) src/DarkMarketAuction.sol:DarkMarketAuction --private-key $(PRIVATE_KEY)
